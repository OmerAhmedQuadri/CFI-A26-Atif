...
class CLI {
    ...
    async getTaskByPriority() {
        try {
            const priority = rs.question('Enter the priority: ') || 'high'
    ...
    createTask = async () => {
        try {
            ...
            const task = rs.question('Enter the task: ')
            const deadline = rs.question('Enter the deadline in (DD/MM/YYYY): ')
            const priority = rs.question('Enter the priority: ')
